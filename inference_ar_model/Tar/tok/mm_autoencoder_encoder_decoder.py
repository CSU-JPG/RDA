...
class MMAutoEncoder(nn.Module):
    ...
    def decode_from_encoder_indices(self, indices, args={}):
        ...
        encoder_x = self.encoder.decode_from_bottleneck(indices)  # encoder for
    ...
    @torch.inference_mode()
    def decode_from_encoder_indices_rec(self, indices, args={}):
        # indices -> encoder feats -> ar -> decoder
        # import pdb; pdb.set_trace()
        encoder_x = self.encoder.decode_from_bottleneck(indices)  # encoder for
        ar_indices = self.ar_sample(encoder_x, args)
        decoder_x, quant, z = self.decoder.decode_from_bottleneck_res(ar_indices)
        x = self.post_process(decoder_x)
        return x, quant, z, ar_indices
    ...
    @torch.inference_mode()
    def forward_res(self, x, args={}):
        # import pdb; pdb.set_trace()
        indices = self.encoder(x.to(self.encoder.dtype))['bottleneck_rep']
