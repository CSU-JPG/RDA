...
class VQVAE(nn.Module):
    ...
    def decode_from_bottleneck_res(self, z):
        if z.ndim == 2:
            b = z.size(0)
            h = w = int(z.size(-1) ** 0.5)
        z, quant = self.model.decode_code_res(z, (b, self.codebook_embed_dim, h, w))
        return self.scale_layer.inv(z), quant, z
...
class VQModel(nn.Module):
    ...
    def decode_code_res(self, code_b, shape=None, channel_first=True):
        quant_b = self.quantize.get_codebook_entry(code_b, shape, channel_first)
        dec, quant = self.decode_res(quant_b)
        return dec, quant
